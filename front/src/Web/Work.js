...
const Work = () => {
    ...
    const handlePage = (value) => {
        ...
        axios({
            method: 'get',
            url: 'http://52.79.155.65:8080/api/works?page=' + pages,     
            headers: {                
                "Authorization": 'Bearer ' + cookies
            }                           
          })
    ...
    const handleButton = () => {
      if(authority === 'ROLE_BENEFICIARY'){
        return(
          <div>
            <Link to={'/DisabledWritePost'}>
                    ...
                    <TableContainer component={Paper} sx={{width: 'auto', marginTop: '20px'}}>
                        <Table sx={{ width: '100%' }} aria-label="simple table">
                            ...
                            {data.contents_responses.content.map(({idx, title, created_date, ended_date}, index) => (
                                ...
                                <Link to={{pathname: `/WorkPostDetails/${idx}`, }} color= 'black'>{title}</Link>
                                </TableCell>
