...
const NoticePatch = ({history, location, match}) => {    
    ...
    useEffect(() => {     
        ...
        axios({
            method: 'get',
            url: 'http://34.64.61.63:8080/api/boards/notice/'+id,                                   
          })
    ...
    const hadleFile = (fileid) => {
        axios({
            method: 'get',
            url: 'http://34.64.61.63:8080/api/boards/download/'+ fileid,                                   
          })
          .then(function (response) {
                // handle success
                console.log(response);
    ...
    const handleButton = (file) => {                      
        if(file && file.length){
            return(
                <div>
                    <a id='files' href={`http://34.64.61.63:8080/api/boards/download/${file}`}>{filename}</a>
    ...
    const handlePatch = () => {
        ...
        axios({
            method: 'patch',
            url: 'http://34.64.61.63:8080/api/boards/'+ id,
            data: {
