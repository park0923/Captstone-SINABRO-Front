...
const NotificationPaginations = (props) => {    
    ...
    useEffect(() => {      
        axios.get('http://54.219.63.255:8080/api/boards')
    ...
    const handlePaging = (number) =>{     
      if(states === ""){
        axios.get('http://54.219.63.255:8080/api/boards?page='+ number + '&size=10')
        .then(function (response) {
            // handle success            
            setData(response.data);
          })
        ...
        axios.get('http://54.219.63.255:8080/api/boards?page='+ number +'&searchOption=title&keyword=' + searchdata)
        .then(function (response) {
            // handle success
            setData(response.data);            
          })
        ...
        axios.get('http://54.219.63.255:8080/api/boards?page='+ number +'searchOption=title_contents&keyword='+ searchdata)
        .then(function (response) {
            // handle success
            setData(response.data);
          })
    ...
    const handleSubmit = (e) => {      
      if(selectvalue === "title"){
        axios.get('http://54.219.63.255:8080/api/boards?searchOption=title&keyword=' + searchdata)
        .then(function (response) {
            // handle success            
        ...
        axios.get('http://54.219.63.255:8080/api/boards?searchOption=title_contents&keyword='+ searchdata)
        .then(function (response) {
            // handle success            
            if(response.data === undefined){
              alert("일치하는 항목이 없습니다.")
  ...
  export default NotificationPaginations;