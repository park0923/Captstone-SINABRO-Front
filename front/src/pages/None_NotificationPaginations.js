...
import None_NoticePost from "./None_NoticePost";

const None_NotificationPaginations = (props) => {
  ...
  useEffect(() => {
    axios
      .get("http://54.219.63.255:8080/api/boards")
  ...
  const handlePaging = (number) => {
    if (states === "") {
      axios
        .get(
          "http://54.219.63.255:8080/api/boards?page=" + number + "&size=10"
        )
        .then(function (response) {
          // handle success
          setData(response.data);
      ...
      axios
        .get(
          "http://54.219.63.255:8080/api/boards?page=" +
            number +
  ...
  for (
    let i = 0;
    i < Math.ceil(data.boards.page.totalElements / data.boards.page.size);
    i++
  ) {
    pageNumbers.push(i);
  }
  ...
  const handleSubmit = (e) => {
    if (selectvalue === "title") {
      axios
        .get(
          "http://54.219.63.255:8080/api/boards?searchOption=title&keyword=" +
            searchdata
        )
        .then(function (response) {
          // handle success
      ...
      axios
        .get(
          "http://54.219.63.255:8080/api/boards?searchOption=title_contents&keyword=" +
            searchdata
        )
        .then(function (response) {
          // handle success
  ...
  return (
    <div>
      <div>
        <div>
          <form
            ...
            className="my-2 flex justify-between appearance-none  relative block w-full font-sebang-gothic px-2 py-2 border-2 border-black"
          >
      ...
      <div>
        <None_NoticePost posts={data.boards.content}></None_NoticePost>
      </div>
      <div>
        <nav>
            ...
            {pageNumbers.map((number) => (
              <div key={number}>
                <button
                  ...
                  className="h-8 w-8 p-1 hover:bg-gray-300 rounded font-sebang-gothic"
                >
                  {number + 1}
                </button>
              </div>
...
export default None_NotificationPaginations;
