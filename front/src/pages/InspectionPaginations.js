...
const InspectionPaginations = () =>{
    const [data, setData] = useState({
      ...
      const handlePaging = (number) =>{     
        if(states === ""){
          axios.get('http://34.64.94.158:8080/api/inspections?page='+ number + '&size=10')
          .then(function (response) {
              // handle success            
              setData(response.data);
            })
          ...
          axios.get('http://34.64.94.158:8080/api/inspections?page='+ number +'&searchOption=title&keyword=' + searchdata)
          .then(function (response) {
              // handle success
              setData(response.data);            
            })
          ...
          axios.get('http://34.64.94.158:8080/api/inspections?page='+ number +'searchOption=title_contents&keyword='+ searchdata)
          .then(function (response) {
              // handle success
              setData(response.data);
            })
      ...
      const handleSubmit = (e) => {      
        if(selectvalue === "title"){          
          axios({
            method: 'get',
            url: 'http://34.64.94.158:8080/api/inspections?searchOption=title&keyword=' + searchdata,            
            headers: {                
                "Authorization": 'Bearer ' + cookies
            }            
          })
          ...
          axios({
            method: 'get',
            url: 'http://34.64.94.158:8080/api/inspections?searchOption=title_contents&keyword='+ searchdata,            
            headers: {                
                "Authorization": 'Bearer ' + cookies
            }            
          })
